...
import { createConversation, deleteConversation, loadCitation, loadHistory, loadMeta, stopTurn, streamQuestion } from './api'
import { renderMarkdown } from './markdown'
import type { ChatMessage, Citation, CitationPage, MetaSuggestion, PublicMeta } from './types'
...
const locale = computed(() => meta.value?.locale || 'zh-CN')
const displayTitle = computed(() => meta.value?.title || '知识问答')
const displayBrand = computed(() => meta.value?.brand || 'PUBLIC QA')
const displayPackID = computed(() => meta.value?.pack_id.toUpperCase() || 'PACK')
const displayPageCount = computed(() => meta.value ? formatNumber(meta.value.page_count) : '—')
const displayDraftCount = computed(() => meta.value ? formatNumber(meta.value.draft_count) : '—')
const displayUnreviewedCount = computed(() => meta.value ? formatNumber(meta.value.unreviewed_count) : '—')
const releaseDetail = computed(() => {
  if (meta.value) return `${meta.value.release_id} · ${meta.value.release_mode}`
  return metaBusy.value ? '正在读取知识快照' : '发布信息暂不可用'
})

onMounted(async () => {
  const tasks: Promise<unknown>[] = [refreshMeta()]
  if (conversations.value.length) tasks.push(selectConversation(conversations.value[0].id))
...
async function refreshMeta(): Promise<void> {
  ...
  try {
    ...
    document.title = `${loaded.title} · ${loaded.brand}`
    document.querySelector<HTMLMetaElement>('meta[name="description"]')?.setAttribute('content', loaded.description)
  } catch (error) {
    meta.value = null
    metaError.value = error instanceof Error ? error.message : '知识包信息暂时无法读取'
  } finally {
...
function formatNumber(value: number): string {
  try { return new Intl.NumberFormat(locale.value).format(value) }
  catch { return String(value) }
}

function formatDate(timestamp: number): string {
  try { return new Intl.DateTimeFormat(locale.value).format(new Date(timestamp)) }
  catch { return new Date(timestamp).toLocaleDateString() }
}
...
async function askSuggestion(suggestion: MetaSuggestion): Promise<void> {
  if (busy.value) return
  query.value = suggestion.question
  await send()
}
  ...
  <div class="observatory-shell">
    <header class="topbar">
      <button class="brand" aria-label="返回问答首页" @click="startNew">
        ...
        <span><strong>{{ displayTitle }}</strong><small>{{ displayBrand }}</small></span>
      </button>
      <div class="release-chip" :class="{ unverified: meta && !meta.corpus_verified }"><span></span><b>{{ displayPageCount }} PAGE PLATE</b><small>{{ releaseDetail }}</small></div>
      ...
      <aside class="history-rail" :class="{ open: showHistory }" aria-label="最近七天对话">
        ...
        <div v-for="item in conversations" :key="item.id" class="history-entry" :class="{ active: item.id === activeID }">
          <button class="history-select" @click="selectConversation(item.id)">
            ...
            <span class="history-copy"><strong>{{ item.preview }}</strong><small>{{ formatDate(item.createdAt) }}</small></span>
          </button>
      ...
      <main class="chat-stage">
        <section ref="messagePane" class="message-pane" aria-live="polite">
          <div v-if="!messages.length" class="welcome-block">
            <div class="hero-grid">
          <div class="hero-copy">
            <p class="eyebrow">READ-ONLY WIKI · {{ displayPackID }}</p>
            <h1>先对准证据，<br><em>再开始曝光。</em></h1>
            <p class="welcome-copy">{{ meta?.description || (metaBusy ? '正在读取当前知识包说明…' : '输入问题后，系统会从当前可用的只读知识快照中查找证据。') }}</p>
            <div v-if="metaError" class="meta-notice" role="status">
              <span>知识快照信息暂不可用：{{ metaError }}</span>
              <button :disabled="metaBusy" @click="refreshMeta">{{ metaBusy ? '重试中…' : '重试' }}</button>
            </div>
            <div class="knowledge-line" aria-label="知识库范围">
              <span><b>{{ displayPageCount }}</b> 知识页面</span>
              <span><b>{{ displayDraftCount }}</b> 草稿页面</span>
              <span class="review-state"><b>{{ displayUnreviewedCount }}</b> 待人工核验</span>
            </div>
              </div>
              ...
              <div class="plate-edge"><span>STACK / {{ displayPageCount }} PAGES</span><span>{{ meta?.release_mode || 'READ ONLY' }}</span></div>
                  <div class="finder-field">
                    ...
                    <div class="finder-copy">
                      ...
                      <div class="prompt-suggestions">
                        <button v-for="suggestion in meta?.suggestions || []" :key="`${suggestion.category}:${suggestion.question}`" :disabled="busy" @click="askSuggestion(suggestion)"><small>{{ suggestion.category }}</small><strong>{{ suggestion.title }}</strong><span>→</span></button>
                        <p v-if="metaBusy" class="suggestion-state">正在读取建议问题…</p>
                        <p v-else-if="!meta?.suggestions.length" class="suggestion-state">暂无建议问题，可在下方直接输入。</p>
                      </div>
                    </div>
                  </div>
                  <div class="plate-edge"><span>NO WEB / NO EMBEDDING</span><span>{{ meta ? (meta.corpus_verified ? 'CORPUS VERIFIED' : 'CORPUS UNVERIFIED') : 'META PENDING' }}</span></div>
                </div>
              </div>
            </div>
