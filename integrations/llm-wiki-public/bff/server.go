...
func NewServer(config Config, store *Store, upstream *Upstream, pages map[string]ManifestPage, releaseID string) *Server {
	s := &Server{config: config, store: store, upstream: upstream, pages: pages, releaseID: releaseID, global: make(chan struct{}, config.GlobalConcurrency), mux: http.NewServeMux()}
	s.routes()
	return s
}
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{"ok": true, "release_id": s.releaseID, "page_count": len(s.pages)})
}
