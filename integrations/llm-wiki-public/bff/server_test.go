...
func TestPublicStreamDoesNotEmitDoneBeforeLaterError(t *testing.T) {
	...
	server := NewServer(config, store, NewUpstream(config), map[string]ManifestPage{}, "release")
...
func TestAnonymousPrincipalIsReleaseScoped(t *testing.T) {
	...
	serverA := NewServer(config, store, NewUpstream(config), map[string]ManifestPage{}, "release-a")
	serverB := NewServer(config, store, NewUpstream(config), map[string]ManifestPage{}, "release-b")
